...
const catalogEntries = [{ id: "my-model", name: "My Model", context: 128000, maxTokens: 16384, input: ["text"], reasoning: false, reasoningLevels: [], capabilities: ["text"] }];
const modelCatalog = {
  ...
  try {
    ...
    await page.addInitScript(({ appConfig, dshStatus, modelConfig, modelCatalog }) => {
      ...
      const handlers = {
        ...
        model_config_save: ({ config }) => { currentModelConfig = structuredClone(config); window.__auditSavedConfigs.push(structuredClone(config)); return new Promise((resolve) => setTimeout(() => resolve(null), 350)); },
        ...
        model_test_connection: (args) => { window.__auditTestCalls.push(structuredClone(args)); return new Promise((resolve) => setTimeout(() => resolve(null), 450)); },
    ...
    assert.equal(await baseURL.inputValue(), invalidRaw, "invalid input must not be normalized or rewritten");
    ...
    // Replace the invalid value in one edit; do not pass through an empty value that has different validation semantics.
    ...
    assert.equal(await saveButton.isEnabled(), true, "changed valid endpoint should restore Save provider");
    ...
    assert.equal(testCalls[0].baseUrl, "https://gateway.example.com/v2");
    await page.waitForTimeout(900);
