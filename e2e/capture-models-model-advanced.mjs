// Models -> Edit provider -> Model settings -> Advanced UI audit capture.
// Scope: one selected model's advanced settings and persistence only.
...
const OUT_DIR = resolve(ROOT, ".artifacts", "e2e", "models-model-advanced");
...
const catalogEntries = [
  ...
  { id: MODEL_ID, name: "DeepSeek Reasoner", context: 65536, maxTokens: 8192, input: ["text", "image"], reasoning: true, reasoningLevels: ["low", "medium", "high"], capabilities: ["text", "reasoning", "vision", "pdf"] },
];
const modelCatalog = {
...
async function main() {
  ...
  try {
    ...
    const row = dialog.locator(`[data-model-id="${MODEL_ID}"]`);
    ...
    await page.screenshot({ path: resolve(OUT_DIR, "models-model-advanced-before.png"), fullPage: true });

    await advanced.click();
    const panel = row.getByTestId("model-advanced");
    await panel.waitFor({ state: "visible" });
    await page.waitForTimeout(500);
    await page.screenshot({ path: resolve(OUT_DIR, "models-model-advanced-open.png"), fullPage: true });

    await panel.getByRole("textbox", { name: "Alias" }).fill("Reasoner Preview");
    await panel.getByRole("spinbutton", { name: "Context window" }).fill("131072");
    await panel.getByRole("spinbutton", { name: "Max output" }).fill("32768");
    const thinking = panel.getByRole("group", { name: "Thinking levels" });
    await thinking.getByRole("button", { name: "high", exact: true }).click();
    await panel.getByRole("textbox", { name: "Wire spelling for high" }).fill("high");
    await panel.getByRole("combobox", { name: "Image input" }).selectOption("text-image");
    await page.waitForTimeout(500);
    ...
    const savedModel = savedProvider.models.find((model) => model.id === MODEL_ID);
    assert.ok(savedModel);
    assert.equal(savedModel.name, "Reasoner Preview");
    assert.equal(savedModel.contextWindow, 131072);
    assert.equal(savedModel.maxTokens, 32768);
    assert.deepEqual(savedModel.reasoningEfforts, { high: "high" });
    assert.deepEqual(savedModel.input, ["text", "image"]);
    ...
    await page.screenshot({ path: resolve(OUT_DIR, "models-model-advanced.png"), fullPage: true });
    await context.close();
    const recorded = await video.path();
    const stable = resolve(OUT_DIR, "models-model-advanced.webm");
