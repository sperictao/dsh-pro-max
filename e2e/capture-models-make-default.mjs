// Provider row -> Make default UI audit capture.
// Scope: open Models, make a ready non-default provider the default,
// inspect the in-flight feedback, and verify the saved/default UI result.
...
const OUT_DIR = resolve(ROOT, ".artifacts", "e2e", "models-make-default");
...
async function main() {
  ...
  try {
    ...
    const makeDefault = deepseekRow.getByRole("button", { name: "Make default" });
    await makeDefault.waitFor({ state: "visible" });
    assert.equal(await makeDefault.isEnabled(), true);
    await page.screenshot({ path: resolve(OUT_DIR, "models-make-default-before-click.png"), fullPage: true });
    ...
    const stable = resolve(OUT_DIR, "models-make-default.webm");
