// ProviderDialog Test connection failure -> repair -> retry UI audit capture.
...
const OUT_DIR = resolve(ROOT, ".artifacts", "e2e", "models-test-connection-retry");
...
async function main() {
  ...
  try {
    ...
    await page.addInitScript(({ appConfig, dshStatus, modelConfig, modelCatalog }) => {
      ...
      const currentModelConfig = structuredClone(modelConfig);
      window.__auditTestAttempts = 0;
      window.__auditTestPending = false;
    ...
    await page.waitForFunction(() => window.__auditTestPending === true);
    await page.waitForFunction(() => window.__auditTestAttempts === 1 && window.__auditTestPending === false);

    const failed = dialog.getByText("HTTP 401 Unauthorized: invalid API key", { exact: true });
    ...
    await page.screenshot({
      path: resolve(OUT_DIR, "models-test-connection-retry-failed.png"),
      fullPage: true,
    });
    ...
    await page.waitForTimeout(700);
    await page.screenshot({
      path: resolve(OUT_DIR, "models-test-connection-retry-repaired.png"),
      fullPage: true,
    });

    await dialog.getByRole("button", { name: "Test connection" }).click();
    await dialog.getByRole("button", { name: "Testing…" }).waitFor({ state: "visible" });
    await page.waitForFunction(() => window.__auditTestAttempts === 2 && window.__auditTestPending === false);

    const success = dialog.getByText("Connection successful", { exact: true });
    ...
    assert.equal(calls[1].api, "openai-completions");
    assert.equal(calls[0].model, "deepseek-chat");
    assert.equal(calls[1].model, "deepseek-chat");
    assert.equal(pageErrors.length, 0, pageErrors.join("\n"));

    await page.waitForTimeout(900);
    await page.screenshot({
      path: resolve(OUT_DIR, "models-test-connection-retry-success.png"),
      fullPage: true,
    });

    await context.close();
    const recorded = await video.path();
    const stable = resolve(OUT_DIR, "models-test-connection-retry.webm");
    if (recorded !== stable) renameSync(recorded, stable);
    console.log(`capture: ${stable}`);
    console.log("audit: 401 visible -> repaired credential clears stale error -> retry succeeds -> zero saves");
  } finally {
