// A/B capture for #48. This script intentionally works on both the #47 baseline
// and the #48 head so the two GIFs use exactly the same interaction sequence.
...
async function main() {
  ...
  try {
    ...
    const failureCount = await page.getByText("Failed to write settings.yaml", { exact: true }).count();
    console.log(`${LABEL}: failure-surfaces=${failureCount}`);
    ...
    const staleCount = await page.getByText("Failed to write settings.yaml", { exact: true }).count();
    console.log(`${LABEL}: stale-failure-surfaces-after-edit=${staleCount}`);
