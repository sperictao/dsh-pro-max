...
export const zhCN: Record<I18nKey, string> = {
  ...
  "Add model": "添加模型",
  "Search models…": "搜索模型…",
  "Model suggestions": "模型候选",
  "No matching models": "没有匹配的模型",
  "Fetching…": "拉取中…",
  "Fetch models": "拉取模型列表",
  "Available upstream models": "上游可用模型",
  "Upstream returned no models": "上游未返回模型",
  "Provider API key environment variable is not configured": "未配置供应商 API 密钥环境变量名",
  "Environment variable is not set in the environment where dsh-pro-max was launched": "该环境变量未在 dsh-pro-max 的启动环境中设置",
  "Failed to reach the provider models endpoint": "访问供应商模型列表端点失败",
  "The provider models endpoint returned an HTTP error": "供应商模型列表端点返回了 HTTP 错误",
  "Failed to read the models response": "读取模型列表响应失败",
  "Provider base URL is required to fetch models": "拉取模型列表需要先填写 Base URL",
  "Failed to reach the model catalog": "拉取模型目录失败",
