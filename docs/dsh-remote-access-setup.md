...
Launcher 当前验证栈为 **dsh 0.1.2-alpha.4**（与内置授权插件一同钉版发布）。兼容判定按
「支持线」进行，不要求与钉版逐字相等：

- **同线且 ≥ 0.1.2-alpha.4**（如同线更高版本）：兼容，可直接使用。主页会提示
  "Newer than the verified stack"，表示比已验证组合新；插件出现不兼容症状时，
  点主页的 **Repair dsh stack** 一键回到验证栈（重装钉版 dsh + 两个授权插件）
- **低于 0.1.2-alpha.4 或跨线**（如 0.1.1-rc.2、0.1.2-alpha.3、0.1.3-alpha.1）：不兼容，主页出现
...
1. Node.js 与 npm
2. 支持线内的 dsh（缺装或不兼容时自动装回验证栈 0.1.2-alpha.4，见第 3 节）
3. 两个授权插件
