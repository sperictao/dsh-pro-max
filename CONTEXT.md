...
- **桥接状态（Bridge States）** — 桌面形态的运行态：应用未运行 / 未装桥接插件 / 代次不符 / 已装未就绪 / 已连接。名字里不带数目是有意的：这个集合会长，而计数写进术语名就会在每次增删后说谎（这条已经栽过两次——三态写成四态、三条写成四条）。后三态各自是一态而不是笼统的「失败」：代次不符时界面直接说出「期望几、拿到几」；已装未就绪（插件激活了但没能建立 token，故能力路由一条都不注册）说的是真实原因与可查的位置，绝不报「未装」——那会把人引向重装，而重装解决不了环境问题。
...
- **消息（Message）** — 过 IPC 的用户可见文案单元：`key` 是本地化前的英文原文（可含 `{{name}}` 占位符），`args` 是插值值。Rust 只产出这个结构、绝不先把值插进句子——成品句子永远命不中词典里的模板 key。解析点只有前端（`renderMessage`）：命中词典就用译文插值，未命中就用同一组参数就地填出英文原文。
- **Rust 直产文案（`trf`）** — 交给 OS API 的文本（托盘菜单标题、系统通知标题），没有前端渲染点，只能由 Rust 查 `i18n.rs` 的小表并插值后交出。

### 语义边界

- 覆盖壳 UI 与 Rust 侧字符串（托盘、错误消息、时间轴诊断）；vendored dsh 插件与 dsh 自身 UI 的语言不在此域。
- 文案的归属按**渲染点**分，不按内容分：前端渲染的走词典（`Message`），OS 直产的走 Rust 小表（`trf`）。两者共用「key 即英文原文」的约定，但占位符写法不同（词典 `{{name}}`、小表 `{name}`）。
