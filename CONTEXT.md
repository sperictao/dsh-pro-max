...
- **桥接五态（Bridge States）** — 桌面形态的五种运行态：应用未运行 / 未装桥接插件 / 代次不符 / 已装未就绪 / 已连接。后三态各自是一态而不是笼统的「失败」：代次不符时界面直接说出「期望几、拿到几」；已装未就绪（插件激活了但没能建立 token，故能力路由一条都不注册）说的是真实原因与可查的位置，绝不报「未装」——那会把人引向重装，而重装解决不了环境问题。
