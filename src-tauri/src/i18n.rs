...
fn zh_cn(key: &str) -> Option<&'static str> {
    Some(match key {
        ...
        "Installing the latest dsh (@next)…" => "正在安装最新的 dsh (@next)…",
        "Check your network and npm settings, then run npm install -g {package}@next and retry" => "请检查网络与 npm 设置，然后运行 npm install -g {package}@next 并重试",
        ...
        "Remote provider API is not responding: {url}/api/llm.providers" => "远程提供方 API 无响应: {url}/api/llm.providers",
