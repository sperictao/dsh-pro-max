...
use super::auth::{
    any_http_status, parse_extra_logins, rpc_request, serve_status_targets_web,
    tailscale_login_from_status_json, validate_cap_domain, AuthConfig,
};
...
use super::process::{
    ...
    run_capture_lines, stream_chunk_lines, which, win_cmd_line, win_quote,
};
use super::setup::{
...
fn ps_wildcards_translate_ere_for_windows() {
    ...
    assert_eq!(
        wildcards,
        vec!["*profile web*--port 3899*", "*--port 3899*profile web*",]
    );
    assert_eq!(
...
fn validate_cap_domain_accepts_and_rejects() {
    ...
    assert_eq!(
        validate_cap_domain("  sub.example.com  ").unwrap(),
        "sub.example.com"
    );
...
fn log_tail_is_capped_drops_blank_lines_and_missing_file_is_none() {
    ...
    assert_eq!(
        read_log_tail(&log, 2).as_deref(),
        Some("Error: boom\nat frame")
    );
    assert_eq!(
        read_log_tail(&log, 10).as_deref(),
        Some("boot ok\nError: boom\nat frame")
    );
...
fn start_diagnosis_points_rewritten_credentials_at_migration() {
    ...
    let unique = std::time::SystemTime::now()
        ...
        .as_nanos();
    let dir =
        std::env::temp_dir().join(format!("dsh-pro-max-diag-{}-{unique}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let log = dir.join("dsh-web.log");
    std::fs::write(
            &log,
...
fn local_access_url_scopes_parsing_to_the_current_boot_region() {
    ...
    assert_eq!(
        super::start::local_access_url_from_log_contents(super::start::fresh_log_region(
            prior, offset
        )),
        None
    );
    ...
    assert_eq!(
        super::start::fresh_log_region(multibyte, mid_char),
        multibyte
    );
}
...
fn web_profile_compat_removal_preserves_other_entries() {
    ...
    // 唯一条目被清掉后落回 `[]` 空数组（合法 YAML，不留空头文件）
    let only = "# profile patch\n- id: dsh-pro-max-compat\n  name: '@deepseek-ai/dsh-attachment'\n  config: {}\n  # Launcher managed: installed dsh CLI is 0.1.1-rc.2\n";
    assert_eq!(
        remove_web_profile_compat_entry(only),
        "# profile patch\n[]\n"
    );
}
...
fn serve_command_forwards_configured_capabilities() {
    ...
    assert_eq!(
        ...
        vec![
            ...
            "--https=443".to_string(),
            "--bg".to_string(),
            "--accept-app-caps=example.com/cap/dsh-admin".to_string(),
            "3899".to_string(),
        ]
    );
    let both = AuthConfig {
...
fn auth_start_script_binds_loopback_and_exports_allowlist() {
    ...
    assert!(
        web.contains("export DSH_TAILSCALE_ALLOWED_LOGINS='owner@example.com,alice@example.com'")
    );
    assert!(web.contains("export DSH_TAILSCALE_ADMIN_CAPABILITY='example.com/cap/dsh-admin'"));
...
fn parse_dist_tag_query_tolerates_both_shapes_and_filters_non_semver() {
    ...
    let legacy = parse_dist_tag_query(
        r#"{"latest":"0.1.0-rc.7","next":"0.1.0-rc.8","junk":"not-a-version"}"#,
    )
    .unwrap();
    assert_eq!(
        legacy.tags,
    ...
    assert_eq!(
        parse_dist_tag_query("{}").unwrap().tags,
...
fn sort_tag_pairs_by_publish_time_newest_first_timeless_last() {
    ...
    let times: HashMap<String, String> = [
        (
            "0.1.0-rc.7".to_string(),
            "2026-08-17T11:50:59.194Z".to_string(),
        ),
        (
            "0.1.2-rc.1".to_string(),
            "2026-09-03T06:21:52.107Z".to_string(),
        ),
        (
            "0.1.2-alpha.5".to_string(),
            "2026-09-02T08:38:34.797Z".to_string(),
        ),
    ]
    .into_iter()
    .collect();
    ...
    let sub_second: HashMap<String, String> = [
        (
            "0.1.2-alpha.5".to_string(),
            "2026-09-03T06:21:52.500Z".to_string(),
        ),
...
fn installed_version_from_spec_parses_concrete_npm_forms_only() {
    ...
    assert_eq!(
        installed_version_from_spec("dsh-better-sidebar@1.2.3").as_deref(),
        Some("1.2.3")
    );
    assert_eq!(
        installed_version_from_spec("npm:dsh-better-sidebar@1.0.0").as_deref(),
        Some("1.0.0")
    );
    assert_eq!(
        installed_version_from_spec("@scope/pkg@2.3.4").as_deref(),
        Some("2.3.4")
    );
    assert_eq!(
        installed_version_from_spec("npm:@scope/pkg@1.0.0").as_deref(),
        Some("1.0.0")
    );
    // 裸版本值（pnpm 精确保存形态）
    assert_eq!(
        installed_version_from_spec("1.2.3").as_deref(),
        Some("1.2.3")
    );
    assert_eq!(
        installed_version_from_spec("0.1.0-rc.6").as_deref(),
        Some("0.1.0-rc.6")
    );
...
fn installed_version_from_disk_reads_node_modules_facts() {
    ...
    std::fs::write(
        nm.join("no-version").join("package.json"),
        r#"{"name":"x"}"#,
    )
    .unwrap();
    ...
    std::fs::write(
        nm.join("pkg").join("package.json"),
        r#"{"version":"1.0.0"}"#,
    )
    .unwrap();
    std::fs::create_dir_all(nm.join("pkg.name-1_x")).unwrap();
    std::fs::write(
        nm.join("pkg.name-1_x").join("package.json"),
        r#"{"version":"2.0.0"}"#,
    )
    .unwrap();
    std::fs::create_dir_all(nm.join("@scope").join("pkg.name-1_x")).unwrap();
    std::fs::write(
        nm.join("@scope").join("pkg.name-1_x").join("package.json"),
        r#"{"version":"3.0.0"}"#,
    )
    .unwrap();
    assert_eq!(
...
fn installed_version_from_disk_edge_shapes_return_none() {
    ...
    let dir =
        std::env::temp_dir().join(format!("dsh-pro-max-disk-ver-edge-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let nm = dir.join("node_modules");
    ...
    std::fs::write(
        nm.join("numeric-version").join("package.json"),
        r#"{"version":123}"#,
    )
    .unwrap();
    ...
    std::fs::write(
        nm.join("ahead").join("package.json"),
        r#"{"version":"2.0.0"}"#,
    )
    .unwrap();
    assert_eq!(
...
#[test]
fn registry_latest_from_json_reads_version_and_dsh_requirement() {
    use super::market::registry_latest_from_json;
    let plain = registry_latest_from_json(r#"{"name":"x","version":"1.2.3"}"#).unwrap();
    assert_eq!(plain.version, "1.2.3");
    assert_eq!(plain.requires_dsh, None);
    // 包 manifest 的 dsh.engines.dsh 优先
    let with_dsh = registry_latest_from_json(
        r#"{"version":"0.3.14","dsh":{"engines":{"dsh":">=0.1.1"}},"engines":{"dsh":">=0.0.9"}}"#,
    )
    .unwrap();
    assert_eq!(with_dsh.requires_dsh.as_deref(), Some(">=0.1.1"));
    // 顶层 engines.dsh 回退
    let fallback =
        registry_latest_from_json(r#"{"version":"1.0.0","engines":{"dsh":">=0.1.0"}}"#).unwrap();
    assert_eq!(fallback.requires_dsh.as_deref(), Some(">=0.1.0"));
    // version 缺失 / 非语义版本 / 非 JSON → 不采信；空串 engines 按未声明
    assert!(registry_latest_from_json(r#"{"name":"x"}"#).is_none());
    assert!(registry_latest_from_json(r#"{"version":"latest"}"#).is_none());
    assert!(registry_latest_from_json("not json").is_none());
    assert_eq!(
        registry_latest_from_json(r#"{"version":"1.0.0","engines":{"dsh":"  "}}"#)
            .unwrap()
            .requires_dsh,
        None
    );
...
#[test]
fn meets_dsh_minimum_gates_declared_requirements() {
    use super::market::meets_dsh_minimum;
    // 同元组更高 rc 也满足（prerelease 序）；跨线更高版本满足 plain semver
    assert!(meets_dsh_minimum(Some("0.1.2-rc.1"), ">=0.1.1"));
    assert!(meets_dsh_minimum(Some("0.1.1"), ">=0.1.1"));
    assert!(meets_dsh_minimum(Some("0.1.2-rc.2"), ">=0.1.0-rc.8"));
    assert!(meets_dsh_minimum(Some("0.1.1-rc.2"), ">=v0.1.0"));
    // 宿主低于要求 / 声明更高 rc
...
fn release_age_policy_from_yaml_reads_window_and_excludes() {
    use super::market::{release_age_policy_from_yaml, DEFAULT_MINIMUM_RELEASE_AGE_MINUTES};
    // 键缺席 → pnpm 11 内置默认 24h，无豁免
    assert_eq!(
        release_age_policy_from_yaml("packages:\n  - .\n"),
...
fn release_age_excluded_matches_exact_or_bare_name() {
    ...
    let excludes = vec![
        "dsh-better-sidebar@0.18.0".to_string(),
        "@scope/pkg".to_string(),
    ];
    assert!(release_age_excluded(
        &excludes,
        "dsh-better-sidebar",
        "0.18.0"
    ));
...
fn publish_time_from_packument_reads_time_map() {
    ...
    assert_eq!(
        publish_time_from_packument(raw, "0.3.14").as_deref(),
        Some("2026-09-03T23:19:35.082Z")
    );
    // 版本缺席 / time 缺席 / 非 JSON → None
    assert_eq!(publish_time_from_packument(raw, "9.9.9"), None);
    assert_eq!(
        publish_time_from_packument(r#"{"name":"x"}"#, "0.3.14"),
        None
    );
...
fn win_quote_keeps_backslashes_literal() {
    // 反斜杠是字面量：不能翻倍
    assert_eq!(
        win_quote(r"C:\Program Files\nodejs\node.exe"),
        "\"C:\\Program Files\\nodejs\\node.exe\""
    );
    assert_eq!(
        win_quote(r"C:\Windows\System32\cmd.exe"),
...
fn run_capture_lines_streams_lines_from_real_process() {
    ...
    assert!(!timed_out);
    assert!(
        stdout.contains("out1") && stdout.contains("out2"),
        "stdout: {stdout}"
    );
    let got = lines.lock().unwrap();
    assert!(got.contains(&"out1".to_string()), "lines: {got:?}");
    assert!(
        got.contains(&"err1".to_string()),
        "stderr 行也要实时回调: {got:?}"
    );
    assert!(
        got.contains(&"err2".to_string()),
        "\\r 切行后 err2 独立成行: {got:?}"
    );
}
...
fn run_capture_lines_timeout_kills_hung_process() {
    ...
    assert!(
        ...
        "超时必须终止等待（杀进程 + 收尾），实际耗时 {:?}",
        start.elapsed()
    );
}

#[test]
fn timeout_failure_message_is_static_and_action_specific() {
    // 静态 key（无插值）：前端 zh 词典按整句精确命中
    set_current("en");
    assert_eq!(
...
fn remote_settings_probe_posts_the_privileged_rpc_directly() {
    let args = curl_remote_rpc_args("https://etmacminim4.taildde4.ts.net/", "settings/describe");
    let no_proxy = args.iter().position(|arg| arg == "--noproxy").unwrap();
    assert_eq!(args[no_proxy + 1], "*");
...
fn model_config_save_preserves_foreign_keys_and_strips_managed_from_extra() {
    ...
    assert_eq!(
        loaded.providers[0].extra,
        serde_json::json!({ "retryPolicy": { "mode": "normal" } })
    );
    std::fs::remove_dir_all(path.parent().unwrap()).ok();
}
...
fn market_plugin_projection_reads_curated_entry() {
    ...
    assert_eq!(
        p.install_specifier.as_deref(),
        Some("@furongjun1999/dsh-memory")
    );
...
fn market_install_specifier_parses_install_command_token() {
    ...
    for raw in [
        "dsh plugin --profile web remove x",
        "npm install dsh-x",
        "",
        "$(cmd)",
    ] {
...
fn market_plugin_fields_fall_through_and_missing_name_drops_entry() {
    ...
    let p = plugin_from_json(
        &serde_json::json!({ "name": "x", "deprecated": true, "replacement": "y" }),
    )
    .unwrap();
    assert!(p.deprecated);
...
fn market_installed_marks_managed_plugins() {
    ...
    let managed = list
        .iter()
        .find(|p| p.name == "@dsh-external/dsh-auth-tailscale")
        .unwrap();
    assert!(managed.managed);
    let community = list
        .iter()
        .find(|p| p.name == "dsh-better-sidebar")
        .unwrap();
...
#[test]
fn market_installed_reads_disabled_state_from_patch() {
    use super::market::installed_list_from_profile;
    // cordis.patch.yml 的 disabled 覆盖行 → enabled=false；无覆盖行的包
    // 默认启用。覆盖行 id 是包 bundle patch 声明的入口 id，不是包名
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-enabled-{}", std::process::id()));
    std::fs::create_dir_all(dir.join("node_modules").join("dsh-better-sidebar")).unwrap();
    std::fs::write(
    ...
    .unwrap();
    std::fs::create_dir_all(dir.join("node_modules").join("plain-pkg")).unwrap();
    let profile = dir.join("package.json");
    std::fs::write(
        &profile,
        r#"{ "dependencies": { "dsh-better-sidebar": "^1.0.0", "plain-pkg": "^2.0.0" } }"#,
    )
    .unwrap();
    std::fs::write(
    ...
    let bundled = list
        ...
        .find(|p| p.name == "dsh-better-sidebar")
        .unwrap();
    assert!(!bundled.enabled, "覆盖行停用的入口应判 false");
    let plain = list.iter().find(|p| p.name == "plain-pkg").unwrap();
    assert!(plain.enabled, "无覆盖行的普通包默认启用");
    std::fs::remove_dir_all(&dir).ok();
}

#[test]
fn market_installed_reads_version_from_disk_or_spec() {
    use super::market::installed_list_from_profile;
    // 版本事实三路径：磁盘命中优先（范围 spec 靠它参与检测）、磁盘缺位回退
    // spec 精确版本、协议形态恒 None（0.0.0 占位不透出）
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-version-{}", std::process::id()));
    std::fs::create_dir_all(dir.join("node_modules").join("range-pkg")).unwrap();
    std::fs::write(
        dir.join("node_modules")
            .join("range-pkg")
            .join("package.json"),
        r#"{ "name": "range-pkg", "version": "1.4.2" }"#,
    )
    .unwrap();
    // bare-pkg 目录存在但无 package.json → 磁盘不可得，走 spec 回退
    std::fs::create_dir_all(dir.join("node_modules").join("bare-pkg")).unwrap();
    let profile = dir.join("package.json");
    std::fs::write(
        &profile,
        r#"{ "dependencies": {
            "range-pkg": "^1.0.0",
            "bare-pkg": "2.3.4",
            "@scope/thing": "npm:@scope/thing@0.9.1",
            "git-pkg": "github:owner/repo"
        } }"#,
    )
    .unwrap();
    let list = installed_list_from_profile(&profile).expect("list");
    ...
    assert_eq!(
        v("bare-pkg").as_deref(),
        Some("2.3.4"),
        "磁盘缺 package.json 回退 spec 精确版本"
    );
    assert_eq!(
        v("@scope/thing").as_deref(),
        Some("0.9.1"),
        "npm: 前缀剥除后按 spec 精确版本"
    );
    assert_eq!(v("git-pkg"), None, "协议形态不透出版本");
    std::fs::remove_dir_all(&dir).ok();
}

#[test]
fn set_entries_enabled_roundtrips_disabled_rows() {
    use super::market::{patch_row_states, set_entries_enabled};
    let entries = vec![(
        "better-sidebar".to_string(),
        "dsh-better-sidebar".to_string(),
    )];
    let base =
        "- id: other\n  name: keep-me\n- insert:\n    - id: keep-insert\n      name: keep-pkg\n";
    // 停用：追加覆盖行，其余行原样保留
    let disabled = set_entries_enabled(base, &entries, false).unwrap().unwrap();
    assert!(disabled.contains("id: better-sidebar"));
    assert!(disabled.contains("disabled: true"));
    assert!(disabled.contains("keep-me"));
    assert_eq!(
    ...
    assert!(!patch_row_states(&enabled).contains_key("better-sidebar"));
    // 启用后再启用：None
...
fn set_entries_enabled_handles_official_empty_scaffold() {
    ...
    // 组。空层判定必须认它——首个停用覆盖行落在注释头之后，`[]` 占位剥除
    let scaffold = "# Your patch layer for this dsh profile, applied after every bundle layer:\n# a top-level YAML array of loader patch entries (id-targeted config\n# overrides, disables, and insert lists; `!!js` expressions allowed).\n[]\n";
    let entries = vec![(
        "better-sidebar".to_string(),
        "dsh-better-sidebar".to_string(),
    )];
    let disabled = set_entries_enabled(scaffold, &entries, false)
        .unwrap()
        .unwrap();
...
#[test]
fn set_entries_enabled_edits_insert_rows_and_preserves_js_tags() {
    use super::market::{patch_row_states, set_entries_enabled};
    // dsh 的 patch 携带 !!js 表达式标签：编辑必须原样保留（serde_yaml
    // TaggedValue 往返），且不误伤其它入口
    let base = "- insert:\n    - id: better-sidebar\n      name: dsh-better-sidebar\n- id: styled\n  name: dsh-styled\n  config: !!js (x) => x\n";
    let entries = vec![(
        "better-sidebar".to_string(),
        "dsh-better-sidebar".to_string(),
    ...
    let disabled = set_entries_enabled(base, &entries, false).unwrap().unwrap();
    assert!(disabled.contains("!!js"), "!!js 表达式标签必须在往返中保留");
    assert!(disabled.contains("id: better-sidebar") && disabled.contains("disabled: true"));
    assert_eq!(
        patch_row_states(&disabled).get("styled"),
        Some(true).as_ref()
    );
    // insert 行内层停用后，bare 行判定读不到它——覆盖行按入口 id 追加，
    // 两形态互不干扰
    let inner_entries = vec![("styled".to_string(), "dsh-styled".to_string())];
    let disabled_inner = set_entries_enabled(&disabled, &inner_entries, false)
        .unwrap()
        .unwrap();
    assert!(disabled_inner.contains("disabled: true"));
    // 启用 insert 行内层：移除内层 disabled 标记，!!js 依旧保留
    let re_enabled = set_entries_enabled(&disabled_inner, &entries, true)
        .unwrap()
        .unwrap();
    assert!(re_enabled.contains("!!js"), "启用路径同样必须保留 !!js");
    let insert_section = re_enabled.split("- id: styled").next().unwrap();
    assert!(
        !insert_section.contains("disabled: true"),
        "insert 内层的 disabled 应被移除"
    );
}

#[test]
fn claimed_entry_rows_reads_bundle_patch_or_falls_back_to_package_name() {
    use super::market::claimed_entry_rows;
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-claimed-{}", std::process::id()));
    std::fs::create_dir_all(dir.join("node_modules").join("@scope").join("bundled")).unwrap();
    std::fs::write(
            dir.join("node_modules").join("@scope").join("bundled").join("cordis.patch.yml"),
            "- config: {}\n- insert:\n    - id: first-row\n      name: '@scope/bundled'\n    - id: second-row\n",
        )
        .unwrap();
    // insert 行的 (id, name)；name 缺席回退包名
    assert_eq!(
        claimed_entry_rows(&dir, "@scope/bundled"),
        vec![
            ("first-row".to_string(), "@scope/bundled".to_string()),
            ("second-row".to_string(), "@scope/bundled".to_string()),
        ]
    );
    // 无 bundle patch / 畸形 patch / 危险包名 → 包名自claim
    std::fs::create_dir_all(dir.join("node_modules").join("plain")).unwrap();
    assert_eq!(
        claimed_entry_rows(&dir, "plain"),
        vec![("plain".to_string(), "plain".to_string())]
    );
    std::fs::create_dir_all(dir.join("node_modules").join("broken")).unwrap();
    std::fs::write(
        dir.join("node_modules")
            .join("broken")
            .join("cordis.patch.yml"),
    ...
    .unwrap();
    assert_eq!(
        claimed_entry_rows(&dir, "broken"),
        vec![("broken".to_string(), "broken".to_string())]
    );
    assert_eq!(
        claimed_entry_rows(&dir, "../evil"),
        vec![("../evil".to_string(), "../evil".to_string())]
    );
    std::fs::remove_dir_all(&dir).ok();
}
...
fn verify_landed_judges_disk_facts_not_exit_codes() {
    ...
    let before = layer(&["old-pkg"]);
    // npm 形态：新键落盘 → Ok；键不在 before 也不在 after → 未生效
    let after = layer(&["old-pkg", "new-pkg"]);
    assert!(verify_landed("new-pkg@1.0.0", &before, &after).is_ok());
...
#[test]
fn row_mounted_names_respects_enablement() {
    use super::market::row_mounted_names;
    use std::collections::BTreeMap;
    let raw = "- id: row-a\n  name: pkg-a\n- id: row-b\n  name: pkg-b\n  disabled: true\n- insert:\n    - id: row-c\n      name: pkg-c\n";
    let mut disabled = BTreeMap::new();
    disabled.insert("row-b".to_string(), false);
    assert_eq!(
        row_mounted_names(raw, &disabled),
        vec!["pkg-a".to_string(), "pkg-c".to_string()]
    );
    // 无停用事实：bare 行与 insert 行全部计挂载
    assert_eq!(row_mounted_names(raw, &BTreeMap::new()).len(), 3);
}

#[test]
fn duplicate_mount_strips_only_new_and_row_mounted() {
    use super::market::duplicate_mount_strips_of;
    use std::collections::BTreeSet;
    let mounted: BTreeSet<String> = ["pkg-a", "pkg-old"].iter().map(|s| s.to_string()).collect();
    let before = vec!["pkg-old".to_string()];
    let after = vec![
        "pkg-old".to_string(),
        "pkg-a".to_string(),
        "pkg-new".to_string(),
    ];
    // 只剥「本次新增且已由 patch 行挂载」：既有条目与无行挂载的新条目不动
    assert_eq!(
        duplicate_mount_strips_of(&before, &after, &mounted),
        vec!["pkg-a".to_string()]
    );
}

#[test]
fn install_notice_serializes_tagged_shape() {
    use super::market::InstallNotice;
    let json = serde_json::to_value(InstallNotice::StrippedDuplicateBundle {
        name: "pkg-a".to_string(),
    })
    .unwrap();
    assert_eq!(
        json,
        serde_json::json!({ "kind": "strippedDuplicateBundle", "name": "pkg-a" })
    );
}
...
fn market_fetch_real_catalog_smoke() {
    ...
    assert!(catalog
        .plugins
        .iter()
        .any(|p| p.install_specifier.is_some()));
    // 弃用标记是目录透传字段（正常条目缺席），是否出现取决于上游内容，
    // 不在此断言；其投影行为由 fixture 单测覆盖
    assert!(catalog
        .plugins
        .iter()
        .any(|p| p.description.as_ref().is_some_and(|d| !d.is_empty())));
...
fn blocked_build_packages_parses_pnpm_stderr() {
    ...
    // 多包逗号分隔
    let multi =
        "\u{2570}\u{2500}\u{25b6} Ignored build scripts: node-pty@1.1.0, simple-git-hooks@2.14.0";
    assert_eq!(
        blocked_build_packages(multi),
        vec!["node-pty", "simple-git-hooks"]
    );
    ...
    assert_eq!(
        blocked_build_packages("Ignored build scripts: dsh-x"),
        vec!["dsh-x"]
    );
    assert_eq!(
        blocked_build_packages("Ignored build scripts: @scope/pkg"),
        vec!["@scope/pkg"]
    );
...
fn failure_raw_merges_stdout_and_stderr_for_fingerprint_detection() {
    ...
    assert_eq!(
        blocked_build_packages(&failure_raw(stdout, "", "add")),
        vec!["node-pty"]
    );
...
fn merge_allow_builds_creates_merges_and_is_idempotent() {
    ...
    assert!(
        raw.contains("allowBuilds") && raw.contains("node-pty: true"),
        "raw: {raw}"
    );
...
fn merge_allow_builds_overwrites_interactive_placeholder() {
    let dir = std::env::temp_dir().join(format!(
        "dsh-pro-max-allow-builds-placeholder-{}",
        std::process::id()
    ));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("pnpm-workspace.yaml");
...
fn install_outcome_serializes_tagged() {
    ...
    let v: serde_json::Value = serde_json::to_value(&InstallOutcome::Installed {
        receipt: None,
        notices: vec![],
    })
    .unwrap();
...
fn catalog_from_raw_validates_structure_and_flags_snapshot() {
    ...
    for raw in [
        r#"{"schemaVersion": 2}"#,
        r#"{"schemaVersion": 1, "repositories": []}"#,
        r#"{"plugins": []}"#,
    ] {
        let err = catalog_from_raw(raw, false).expect_err("should reject");
        assert!(
            matches!(err, CatalogLoadError::UnsupportedSchema(_)),
            "unexpected error for {raw}"
        );
    }
    // 快照标记原样透传
    assert!(
        catalog_from_raw(catalog_fixture(), true)
            .expect("parse")
            .from_snapshot
    );
}
...
fn package_name_from_specifier_mirrors_frontend_semantics() {
    assert_eq!(
        package_name_from_specifier("pkg@1.0.0").as_deref(),
        Some("pkg")
    );
    assert_eq!(
        package_name_from_specifier("@scope/pkg@1.0").as_deref(),
        Some("@scope/pkg")
    );
    assert_eq!(
        package_name_from_specifier("@scope/pkg").as_deref(),
        Some("@scope/pkg")
    );
    assert_eq!(
        package_name_from_specifier("plain-name").as_deref(),
        Some("plain-name")
    );
    // 协议形态（npm: 别名 / github: / file:）安装后的 dependencies 键不可预知
    assert_eq!(package_name_from_specifier("npm:pkg@latest"), None);
    assert_eq!(
        package_name_from_specifier("github:owner/repo#c0ffee"),
        None
    );
...
fn policy_entries_parse_contract() {
    ...
    // 空数组 = 生效且全拒
    assert_eq!(
        policy_entries_from_raw(r#"{"allowed": []}"#).unwrap(),
        Some(Vec::<String>::new())
    );
    assert_eq!(
...
fn install_failure_message_detects_allowbuilds_block() {
    ...
    let msg = install_failure_message(
        "add",
        "pnpm: Ignored build scripts: dsh-x ... add it under allowBuilds",
    );
...
fn install_receipt_prefers_new_key_then_npm_name() {
    ...
    let r = install_receipt(
        "dsh-new@3.0",
        Some(vec!["old".into(), "dsh-new".into()]),
        &list,
        Some("dsh-new"),
    )
    .unwrap();
...
fn install_receipt_protocol_reinstall_locates_existing_key() {
    ...
    let r = install_receipt(
        ...
        Some(before),
        &list,
        Some(&specifier_to_catalog_name(
            "github:toby-bridges/api-relay-audit",
        )),
    )
    .unwrap();
...
fn specifier_to_catalog_name_mirrors_frontend_semantics() {
    assert_eq!(
        specifier_to_catalog_name("github:toby-bridges/api-relay-audit"),
        "api-relay-audit"
    );
    assert_eq!(
        specifier_to_catalog_name("git+https://github.com/omdsh-dev/dsh-at-file.git"),
        "dsh-at-file.git"
    );
    assert_eq!(specifier_to_catalog_name("@scope/pkg@1.0"), "pkg");
    assert_eq!(specifier_to_catalog_name("dsh-context"), "dsh-context");
    assert_eq!(
        specifier_to_catalog_name("dsh-context@1.2.3"),
        "dsh-context"
    );
}
...
fn poll_local_ready_settles_the_port_bound_then_crash_race() {
    ...
    assert!(matches!(
        poll_local_ready(|| true, || true, || {}, 3, 0),
        LocalReady::Responding
    ));
    // HTTP 从未应答、进程死亡 → Dead（即使端口曾绑定、启动等待已放行）
    assert!(matches!(
        poll_local_ready(|| false, || false, || {}, 3, 0),
        LocalReady::Dead
    ));
    // 进程活着但始终不应答 → Unresponsive
    assert!(matches!(
        poll_local_ready(|| false, || true, || {}, 3, 0),
    ...
    assert!(matches!(
        poll_local_ready(|| false, || false, || {}, 20, 0),
        LocalReady::Dead
    ));
}
...
fn poll_local_ready_soaks_the_serve_then_activation_crash_race() {
    ...
    // 应答后全程稳定 → Responding
    assert!(matches!(
        poll_local_ready(|| true, || true, || {}, 2, 4),
        LocalReady::Responding
    ));
}
...
fn install_decision_success_computes_receipt_from_before_after() {
    ...
    let outcome =
        super::market::install_decision("dsh-new@2.0", Ok(()), Some(vec![]), Some(after), None)
            .expect("installed");
    match outcome {
    ...
    assert!(matches!(
        outcome,
        InstallOutcome::Installed { receipt: None, .. }
    ));
}
...
fn fixture_https_ok(_: &str) -> bool {
    PROBE_HTTPS_OK.with(|c| c.get())
}
fn fixture_ws_ok(_: &str) -> bool {
    PROBE_WS_OK.with(|c| c.get())
}
...
fn fixture_https_ok_via_proxy(_: &str, _: &super::MacosHttpsProxy) -> bool {
    PROBE_PROXIED_HTTPS_OK.with(|c| c.get())
}
fn fixture_ws_ok_via_proxy(_: &str, _: &super::MacosHttpsProxy) -> bool {
    PROBE_PROXIED_WS_OK.with(|c| c.get())
}
fn fixture_active_proxy() -> Option<super::MacosHttpsProxy> {
    PROBE_PROXY.with(|c| c.borrow().clone())
}
...
fn probe_orchestration_classifies_each_failure_ring() {
    ...
    let p = super::probe::probe_remote_url_with(
        url,
        &auth,
        &probe_hooks(false, false, RemoteRpcAccess::Ready, None, false, false),
    );
    ...
    let p = super::probe::probe_remote_url_with(
        url,
        &auth,
        &probe_hooks(true, true, RemoteRpcAccess::Denied, None, false, false),
    );
    ...
    let p = super::probe::probe_remote_url_with(
        url,
        &auth,
        &probe_hooks(true, false, RemoteRpcAccess::Ready, None, false, false),
    );
    assert_eq!(p.access, RemoteUrlAccess::EndpointFailure);
    // 直连全通、无系统代理：就绪
    let p = super::probe::probe_remote_url_with(
        url,
        &auth,
        &probe_hooks(true, true, RemoteRpcAccess::Ready, None, false, false),
    );
    ...
    let p = super::probe::probe_remote_url_with(
        url,
        &auth,
        &probe_hooks(true, true, RemoteRpcAccess::Ready, Some(proxy), true, true),
    );
    assert_eq!(p.access, RemoteUrlAccess::Ready);
}
...
fn pnpm_failure_hint_classifies_known_families() {
    let cases: &[(&str, &str)] = &[
            (
                ...
                "The web profile's node_modules was created by a different pnpm major",
            ),
            (
                "Error: ERR_PNPM_UNEXPECTED_STORE\n  node_modules → /a\n  pnpm now wants → /b",
                "linked to a different pnpm store",
            ),
            (
                "ERR_PNPM_MISSING_TARBALL_INTEGRITY  foo@1.0.0 has no \"integrity\" field",
                "no integrity hash",
            ),
            ("ERR_PNPM_PATCH_FAILED Could not apply patch foo@1.0.0", "pnpm patch in the profile no longer applies"),
            (
                "ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION  Package is too fresh",
                "release-age protection blocked this change",
            ),
            ("ERR_PNPM_NO_MATURE_MATCHING_VERSION", "release-age protection blocked this change"),
            (
                "ERR_PNPM_GIT_DEP_PREPARE_NOT_ALLOWED The git-hosted package needs to execute build scripts",
                "git-hosted plugin needs to run a build script",
            ),
            ("ERR_PNPM_IGNORED_BUILDS Ignored build scripts", "need to run build scripts"),
            ("ERR_PNPM_FETCH_404 GET https://registry.npmjs.org/x - 404", "cannot be resolved from the registry"),
            ("ERR_PNPM_EPERM: operation not permitted, rename", "Windows cannot replace files that are in use"),
            (
                "EPERM: operation not permitted, rename '/x/node_modules/y_tmp_1'",
                "Windows cannot replace files that are in use",
            ),
            ("pnpm not found on PATH", "pnpm was not found"),
            (
                "ENOENT: no such file or directory, open '/x/y'\nwhile installing a direct dependency",
                "no longer exists on disk",
            ),
            ("GET https://x error (23) The operation was aborted due to timeout", "The download timed out"),
            ("ERR_PNPM_FETCH_502 while fetching", "A transient network failure"),
            ("FetchError: socket hang up", "A transient network failure"),
        ];
    for (output, fragment) in cases {
        let hint = super::market::pnpm_failure_hint(output)
            .unwrap_or_else(|| panic!("no hint for {output:?}"));
        assert!(
            hint.contains(fragment),
            "hint for {output:?} should mention {fragment:?}: {hint:?}"
        );
    }
}

#[test]
fn pnpm_failure_hint_misses_unrecognized_output() {
    // 未识别的输出不强行归类：普通失败照旧走原始错误透传
    assert_eq!(
        super::market::pnpm_failure_hint("some random failure"),
        None
    );
    assert_eq!(super::market::pnpm_failure_hint(""), None);
}

#[test]
fn install_failure_message_prefers_classification_over_generic_text() {
    let msg = super::market::install_failure_message("add", "ERR_PNPM_FETCH_404: not found");
    assert!(
        msg.contains("cannot be resolved from the registry"),
        "{msg}"
    );
    // 分类未命中时保持原行为（普通失败透传上游错误）
    let msg = super::market::install_failure_message("add", "weird failure");
    assert!(msg.contains("weird failure"), "{msg}");
}
...
#[test]
fn run_capture_lines_untriggered_cancel_is_transparent() {
    // 未置位的令牌不改变行为（取消是可选通道，不是必经状态）
    let Some(node) = which("node") else { return };
...
#[test]
fn compat_cache_roundtrip_and_schema_guard() {
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-compat-cache-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("market-compat-cache.json");
    let mut entries = std::collections::BTreeMap::new();
    entries.insert(
        "dsh-better-sidebar".to_string(),
        super::market::CompatCacheEntry {
            checked_at: 1000,
            requires_dsh: Some(">=0.2.0".to_string()),
        },
    );
    super::market::write_compat_cache_file(&path, &entries).unwrap();
    let loaded = super::market::load_compat_cache_file(&path).unwrap();
    assert_eq!(loaded.schema, super::market::COMPAT_CACHE_SCHEMA);
    assert_eq!(
        loaded.entries["dsh-better-sidebar"].requires_dsh.as_deref(),
        Some(">=0.2.0")
    );
    // 旧契约/损坏内容按无缓存处理（与目录快照同一哲学）
    std::fs::write(&path, "{\"schema\":\"legacy\",\"entries\":{}}").unwrap();
    assert!(super::market::load_compat_cache_file(&path).is_err());
    std::fs::write(&path, "not json").unwrap();
    assert!(super::market::load_compat_cache_file(&path).is_err());
...
#[test]
fn compat_cache_write_prunes_to_hard_cap() {
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-compat-prune-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("market-compat-cache.json");
    let mut entries = std::collections::BTreeMap::new();
    for i in 0..(super::market::COMPAT_CACHE_MAX_ENTRIES + 100) {
        entries.insert(
            format!("pkg-{i}"),
            super::market::CompatCacheEntry {
                checked_at: i as i64,
    ...
    assert_eq!(
        loaded.entries.len(),
        super::market::COMPAT_CACHE_MAX_ENTRIES
    );
    // 淘汰的是最旧的：最小的 checked_at 不在缓存里
    assert!(!loaded.entries.contains_key("pkg-0"));
    assert!(loaded.entries.contains_key(&format!(
        "pkg-{}",
        super::market::COMPAT_CACHE_MAX_ENTRIES + 99
    )));
    let _ = std::fs::remove_dir_all(&dir);
}
...
#[test]
fn valid_repo_id_rejects_non_owner_repo_shapes() {
    assert!(super::market::valid_repo_id("omdsh-dev/DSH-better-sidebar"));
    assert!(super::market::valid_repo_id("scope.name/pkg_name-1"));
    assert!(!super::market::valid_repo_id("only-one-segment"));
    assert!(!super::market::valid_repo_id("a/b/c"));
    assert!(!super::market::valid_repo_id("a/../.."));
    assert!(!super::market::valid_repo_id("/leading"));
    assert!(!super::market::valid_repo_id("a/b/"));
    assert!(!super::market::valid_repo_id("https://evil.com/a/b"));
    assert!(!super::market::valid_repo_id(""));
}

#[test]
fn release_notes_parse_reads_probe_payload() {
    // 真实数据形态（awesome-dsh-plugin.com/updates.json 探针产物）
    let body = r##"{
            "updates": {
                "https://github.com/o/r": {
                    "release": {"tag": "v0.4.3", "name": "v0.4.3", "publishedAt": "2026-09-04T13:56:57Z",
                                "url": "https://github.com/o/r/releases/tag/v0.4.3", "body": "# notes"},
                    "commits": [
                        {"sha": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", "message": "build: 0.4.3", "date": "2026-09-04T13:51:47Z"},
                        {"sha": "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb", "message": "docs: x", "date": "2026-09-04T12:00:00Z"}
                    ]
                },
                "https://github.com/o/no-release": {"release": null, "commits": []}
            }
        }"##;
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-notes-parse-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("market-updates-cache.json");
    std::fs::write(&path, body).unwrap();
    let notes = super::market::release_notes_once("o/r", Some(&path)).unwrap();
    let release = notes.release.expect("release present");
    assert_eq!(release.tag.as_deref(), Some("v0.4.3"));
    assert_eq!(release.body, "# notes");
    assert_eq!(notes.commits.len(), 2);
    assert_eq!(
        notes.commits[0].sha,
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
    );
    // 探针未覆盖 → None；release 为 null → commits 仍透传
    assert!(super::market::release_notes_once("o/absent", Some(&path)).is_none());
    let no_release = super::market::release_notes_once("o/no-release", Some(&path)).unwrap();
    assert!(no_release.release.is_none());
    let _ = std::fs::remove_dir_all(&dir);
}

#[test]
fn release_notes_commits_are_capped() {
    let mut commits = String::new();
    for i in 0..40 {
        commits.push_str(&format!(
            "{{\"sha\":\"{i:040}\",\"message\":\"m{i}\",\"date\":null}},"
        ));
    }
    commits.pop();
    let body = format!(
        r#"{{"updates":{{"https://github.com/o/r":{{"release":null,"commits":[{commits}]}}}}}}"#
    );
    let dir = std::env::temp_dir().join(format!("dsh-pro-max-notes-cap-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let path = dir.join("market-updates-cache.json");
    std::fs::write(&path, body).unwrap();
    let notes = super::market::release_notes_once("o/r", Some(&path)).unwrap();
    assert_eq!(notes.commits.len(), super::market::RELEASE_COMMITS_LIMIT);
    let _ = std::fs::remove_dir_all(&dir);
}
...
#[test]
fn parse_dump_entries_reads_layers_and_fields() {
    // 真实 dump-config 形态：`# == <bundle>` 注释切层，- id 行开条目，
    // 两空格 name/disabled 归当前条目，config 子键不干扰
    let stdout = concat!(
        "# == @deepseek-ai/dsh-base\n",
        "- id: timer\n",
    ...
    let entries = super::market::parse_dump_entries(stdout);
    assert_eq!(entries.len(), 4);
    assert_eq!(entries[0].id, "timer");
    assert_eq!(entries[0].layer, "@deepseek-ai/dsh-base");
    assert!(entries[1].disabled, "disabled 字段要归到当前条目");
    assert_eq!(
        entries[1].name.as_deref(),
        Some("@deepseek-ai/cordis-plugin-hmr")
    );
    // config 子键（两空格但非 name/disabled）不被误读
    assert_eq!(entries[3].id, "solo");
    assert_eq!(entries[3].name, None);
}

#[test]
fn parse_orphan_warnings_reads_loader_verbatim() {
    // 本机 dump-config 的真实 stderr 原话
    let stderr = "dsh: [/Users/x/.dsh/profiles/web/cordis.patch.yml] patch: entry \"dsh-pro-max-compat\" not found\n\
dsh: [yml] patch: entry \"dsh-pro-max-compat\" not found\n";
    assert_eq!(
        super::market::parse_orphan_warnings(stderr),
        vec!["dsh-pro-max-compat".to_string()]
    );
    assert!(super::market::parse_orphan_warnings("no warnings here").is_empty());
}

#[test]
fn diagnostics_from_dump_flags_duplicates_and_orphans() {
    let stdout = concat!(
        "# == base\n",
        "- id: timer\n",
        "  name: a\n",
    ...
    let stderr = "dsh: [x] patch: entry \"ghost\" not found\n";
    let d = super::market::diagnostics_from_dump(stdout, stderr);
    assert_eq!(d.entries, 3);
    assert_eq!(d.disabled, 1);
    assert_eq!(d.duplicates.len(), 1);
    assert_eq!(d.duplicates[0].id, "timer");
    assert_eq!(d.duplicates[0].count, 2);
    assert_eq!(
        d.duplicates[0].layers,
