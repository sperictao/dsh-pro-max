...
pub(crate) fn render_start_web(
    ...
    let admin_cap_line = auth
        ...
        .unwrap_or_default();
    // allowlist 行仅在确有登录名时输出；无登录名的自启 = 授权插件默认
    // deny-all，本机 launch token 访问不受影响
    let login_line = auth
        .allowed_logins(Some(login))
        .map(|logins| format!("export {TAILSCALE_LOGIN_ENV}={}\n", sh_quote(&logins)))
        .unwrap_or_default();
    format!(
        "#!/bin/sh\n# generated by DSH Pro Max; do not edit\nif {node} -e {guard}; then exit 0; fi\nexport PATH={node_dir}:$PATH\n{login_line}{use_cap_line}{admin_cap_line}exec {dsh} --profile web --host 127.0.0.1 --port {port} --no-open{trusted}\n",
        node = sh_quote(node),
...
pub(crate) fn autostart_impl(enabled: bool) -> Result<(), String> {
    ...
    if enabled {
        ...
        let login_line = auth
            .allowed_logins(Some(&login))
            .map(|logins| format!("set \"{TAILSCALE_LOGIN_ENV}={logins}\"\r\n"))
            .unwrap_or_default();
        let web = format!(
            "@echo off\r\nrem generated by DSH Pro Max; do not edit\r\nset \"PATH={node_dir};%PATH%\"\r\n\"{node}\" -e \"{guard}\" >nul 2>&1\r\nif %errorlevel%==0 exit /b 0\r\n{login_line}{use_cap_line}{admin_cap_line}call \"{dsh}\" --profile web --host 127.0.0.1 --port {port}{trusted}\r\n",
            node_dir = node_dir,
            node = node,
